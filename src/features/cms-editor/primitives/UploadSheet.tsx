...
export type UploadSheetHandle = {
  openFilePicker: () => void;
};

export const UploadSheet = forwardRef<
  UploadSheetHandle,
  {
    ...
    /** Hide inline trigger UI — use ref.openFilePicker() instead */
    headless?: boolean;
  }
>(function UploadSheet(
  ...
  const uploadBlob = useCallback(
    async (blob: Blob, filename: string) => {
      ...
      if (!response.ok || !data.url) {
        throw new Error(typeof data?.message === "string" ? data.message : "Feltöltés sikertelen");
      }
      onUploaded(data.url);
    },
    [onUploaded]
  ...
  return (
    <div className={headless ? "contents" : "space-y-2"}>
      {!headless && usageLabel ? (
        <p className="text-[10px] uppercase tracking-widest text-muted-foreground">
          Cél: {usageLabel}
        </p>
      ) : null}
      {!headless && recommendedSize ? (
        <p className="text-[10px] uppercase tracking-widest text-muted-foreground">
      ...
      {headless ? (
        ...
        <label className="inline-flex items-center gap-2 text-xs cursor-pointer">
          <span className="inline-flex h-8 items-center rounded-md border px-3 hover:bg-muted">
            {label}
          </span>
          <input
            ref={fileInputRef}
      ...
      {imageSource ? (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center p-4 md:p-8">
            ...
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-black/20 gap-4">
              <div>
                ...
                <p className="text-[10px] text-neutral-500 uppercase font-black tracking-widest">
                  {useFullImage ? "Teljes kép feltöltése" : "Válaszd ki a kivágás alakját"}
                </p>
              </div>
            ...
            {useFullImage ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 bg-black text-center min-h-0">
                <p className="text-sm text-neutral-300 max-w-md">
                  A teljes kép feltöltődik — nincs kivágás.
                </p>
                {fileMeta ? (
            ...
            <div className="p-6 bg-black/40 border-t border-white/5 space-y-6 shrink-0">
              ...
              <div className="flex justify-end gap-4">
                ...
                <button
                  ...
                  className="h-10 px-6 bg-white text-black rounded-xl uppercase tracking-widest text-xs font-bold inline-flex items-center gap-2 disabled:opacity-50"
                >
